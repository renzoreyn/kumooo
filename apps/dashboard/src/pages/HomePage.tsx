...
import type { SitePlanLimits } from "../lib/api/sites";
...
export function HomePage() {
  ...
  async function load() {
    if (!org) return;
    const r = await api.sites(org.id);
    setSites(r.sites);
  ...
  async function createSite(e: React.FormEvent) {
    e.preventDefault();
    if (!org || atLimit) return;
    setError(null);
    try {
      const { site, limits: next } = await api.createSite(org.id, { name });
      setSites((s) => [...s, site]);
    ...
    <Shell title="Sites">
      {sites.length === 0 ? (
        <div className="card" style={{ marginBottom: "1.25rem" }}>
          ...
          <p className="muted">Free plan includes 2 sites. Let's build one.</p>
        </div>
      ) : (
            ...
            >
              ...
              <div>
                ...
                <div className="muted" style={{ fontSize: "0.9rem" }}>
                  {s.slug}.kumooo.dev · theme: {s.theme}
                </div>
              </div>
            </Link>
          ))}
        ...
        <p className="muted" style={{ marginBottom: "0.75rem" }}>
          {limits.maxSites === null
            ...
            : `Free plan: ${limits.usedSites} / ${limits.maxSites} sites used.${
                atLimit ? " Archive or delete one to create another." : ""
              }`}
        </p>
      ) : null}
      <form
      ...
      >
        ...
        <button className="btn primary" type="submit" disabled={atLimit}>
