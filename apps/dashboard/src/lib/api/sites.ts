...
export const sitesApi = {
  list: (orgId: string, status?: "archived" | "all") => {
    ...
    return request<{ sites: Site[] }>(`/v1/orgs/${orgId}/sites${q}`);
  },
  ...
  create: (
    ...
    request<{ site: Site }>(`/v1/orgs/${orgId}/sites`, {
      method: "POST",
  ...
  restore: (siteId: string) =>
    request<{ ok: boolean; status: string }>(`/v1/sites/${siteId}/restore`, { method: "POST" }),
